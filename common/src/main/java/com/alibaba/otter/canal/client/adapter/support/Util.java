...
public class Util {
    ...
    public static File getConfDirPath(String subConf) {
        ...
        File file = null;
        if (path != null) {
            file = new File(path + ".." + File.separator + Constant.CONF_DIR + File.separator + StringUtils.trimToEmpty(subConf));
            if (!file.exists()) {
                file = new File(path + StringUtils.trimToEmpty(subConf));
            }
        }
        if (subConf.equals("rdb")) {
            file = new File("E:\\conf\\rdb\\my_canal.yml");
        }
